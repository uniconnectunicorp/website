...
import { X, BookOpen, Clock, Award, TrendingUp, CheckCircle, Zap, Target, Sparkles, ChevronRight, Star, GraduationCap, Users, Calendar } from 'lucide-react';
import Link from 'next/link';
import { EnrollmentFormV2 } from '@/components/forms/EnrollmentFormV2';

export default function CourseDetailPopup({ course }) {
  ...
  useEffect(() => {
    ...
    const checkAndShowPopup = () => {
      const lastShown = localStorage.getItem(`courseDetailPopup_${course.slug}_lastShown`);
  ...
  const handleClose = () => {
    setIsOpen(false);
    if (course) {
      localStorage.setItem(`courseDetailPopup_${course.slug}_lastShown`, new Date().toDateString());
    }
  };
  ...
  const getDuration = () => {
    if (course.maxTime) {
      return `${course.minTime}-${course.maxTime} meses`;
    }
    return `A partir de ${course.minTime} meses`;
  };

  const highlights = [
    { icon: Clock, text: getDuration(), label: 'Duração' },
    { icon: Award, text: 'Certificado MEC', label: 'Reconhecido' },
    { icon: Users, text: course.modality || 'Online', label: 'Modalidade' }
  ];
                      ...
                      >
                        <div className="relative">
                          ...
                          <motion.div
                            ...
                            className="absolute -top-3 -right-3 bg-[#ff6600] rounded-full p-2 shadow-lg"
                          >
                          ...
                          >
                            <Sparkles className="h-8 w-8 text-[#ff6600] drop-shadow-lg" />
                          </motion.div>
                        </div>
                      </motion.div>
                      ...
                      >
                        ...
                        {course.rating && (
                          <motion.div
                            ...
                            className="flex items-center justify-center lg:justify-start gap-2 text-yellow-500"
                          >
                            {[...Array(5)].map((_, i) => (
                              <Star key={i} className="h-5 w-5 fill-current" />
                            ))}
                            <span className="text-gray-700 font-semibold ml-2">{course.rating || '4.9'}/5.0</span>
                          </motion.div>
                        )}
                      </motion.div>
                    </div>
