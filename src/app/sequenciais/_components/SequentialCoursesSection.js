...
import { SequentialCourseCard } from '@/components/cards/SequentialCourseCard';
import { loadMoreSequential } from '@/data/server-actions/loadMoreSequential';
import { useState, useEffect } from 'react';

export default function SequentialCoursesSection({ coursesQuantity, initialCourses }) {
    ...
    const handleLoadMore = async () => {
        ...
        const newCourses = await loadMoreSequential(nextPage); 
        setLoading(false)
                ...
                >
                    ...
                    {loading && (
                        <div className="absolute inset-0 flex items-center justify-center">
                            <div className="h-5 w-5 border-2 border-[#0b3b75] border-t-transparent rounded-full animate-spin" />
                        </div>
                    )}
                </Button>
                )}
